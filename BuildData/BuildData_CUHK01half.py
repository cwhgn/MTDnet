...
def BuildTripleFile(txtdir,name,ids):
    ...
    pos1_list = open(txtdir+'/CUHK01half_%s_pos1.txt' %name, 'w')
    for x in range(0,list.shape[0]):
        pos1_list.write('%s 0\n' %list[x,0])
    pos1_list.close()
    
    pos2_list = open(txtdir+'/CUHK01half_%s_pos2.txt' %name, 'w')
    for x in range(0,list.shape[0]):
        pos2_list.write('%s 1\n' %list[x,1])
    pos2_list.close()
    
    neg_list = open(txtdir+'/CUHK01half_%s_neg.txt' %name, 'w')
    for x in range(0,list.shape[0]):
        neg_list.write('%s 0\n' %list[x,2])
    neg_list.close()
...
def BuildRankFile(txtdir,name,ids):
    list=[]
    randstart=np.random.randint(0,485-ids)#used to control which part of val data is used to rank the accuracy
    for id in xrange(randstart,ids+randstart):
        for i in xrange(0,2):
            if not os.path.exists(outdir+'/'+name+'/'+'CamA_%04d_%02d.jpg' %(id+1,i+1)): 
                print 'No image is find!'
            count = 0;
            for id_p in xrange(randstart,ids+randstart):
                label=0;
                if id_p==id:
                    label=1;
            #for j in xrange(0,1):
    ...
    pos_list = open(txtdir+'/CUHK01half_%srank_pos.txt' %name, 'w')
    for x in range(0,list.shape[0]):
        pos_list.write('%s %d\n' %(list[x,0],int(list[x,2])))
    pos_list.close()
    
    neg_list = open(txtdir+'/CUHK01half_%srank_neg.txt' %name, 'w')
    for x in range(0,list.shape[0]):
...
if __name__ == '__main__':
    ...
    txtdir = '.../BuildData/CUHK01halfData'
    #output path for three folders (train,val,test)
    outdir = '.../BuildData/CUHK01halfData/image'
    ...
    val_data = np.load(r'%s/RecordedValdata_CUHK01half.npy' %(valdir))#Load if used our recorded val data
