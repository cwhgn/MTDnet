...
if __name__ == '__main__':
    ...
    for idx in xrange(0,test_num): 
        tmp_feats=[]
        for idx2 in xrange(1,100):
            if not os.path.exists('%s/%04d_%02d.jpg' %(indir,idx+1,idx2+1)):
            ...
            ima_path='%s/%04d_%02d.jpg' %(indir,idx+1,idx2+1)
            #get feat
    ...
    for idx in xrange(0,test_num): 
        tmp_feats=[]
        for idx2 in xrange(1,100):
            if not os.path.exists('%s/%04d_%02d_mirror.jpg' %(indir,idx+1,idx2+1)):
                #print 'Image %s/CamA_%04d_%02d.jpg does not exist!' %(indir,idx+1,idx2+1)
                break
            #get image
            #print 'Image %s/CamA_%04d_%02d.jpg is processing...' %(indir,idx+1,idx2+1)
            ima_path='%s/%04d_%02d_mirror.jpg' %(indir,idx+1,idx2+1)
            #get feat
            extract_feat = ExtractFeat(net,ima_path)
            tmp_feat =  copy.deepcopy(extract_feat)
            #print 'tmp_feat shape is', tmp_feat.shape
    ...
    for idx in xrange(0,test_num): 
        tmp_feats=[]
        for idx2 in xrange(0,1):
            if not os.path.exists('%s/%04d_%02d.jpg' %(indir,idx+1,idx2+1)):
                break
            #get image
            imb_path='%s/%04d_%02d.jpg' %(indir,idx+1,idx2+1)
            #get feat
    ...
    for idx in xrange(0,test_num): 
        ...
        for idx2 in xrange(0,1):
            if not os.path.exists('%s/%04d_%02d_mirror.jpg' %(indir,idx+1,idx2+1)):
                break
            #get image
            imb_path='%s/%04d_%02d_mirror.jpg' %(indir,idx+1,idx2+1)
            #get feat
            extract_feat = ExtractFeat(net,imb_path)
            tmp_feat =  copy.deepcopy(extract_feat)
            #print 'tmp_feat shape is', tmp_feat.shape
