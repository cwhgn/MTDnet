...
def GetRanks(net,indir,pidx,idx):
    tmp_ranks=[]
    for n in xrange(0,test_num):
        ...
        for idx2 in xrange(0,2):
            ...
            for n2 in xrange(0,2):
                bpath = indir+'/'+'CamB_%04d_%02d.jpg' %(n+1,n2+1)
                bpath_m = indir+'/'+'CamB_%04d_%02d_mirror.jpg' %(n+1,n2+1)
                if not os.path.exists(bpath):
                    print 'Image CamB_%04d_%02d.jpg is not existed.' %(n+1,n2+1)
                if not os.path.exists(bpath_m):
                    print 'Image CamB_%04d_%02d_mirror.jpg is not existed.' %(n+1,n2+1)
                ...
                sumscore_mavg = sumscore_mavg + score_mavg
                sumscore_avg = sumscore_avg + score_avg
                #print idx2,n2,score_mavg,score_avg
        sumscore_mavg = sumscore_mavg/4.0
        sumscore_avg = sumscore_avg/4.0
        #print '----',sumscore_mavg,sumscore_avg;
        tmp_ranks.append((n,sumscore_mavg,sumscore_avg))
    ...
    print 'ID %d image %d best rank is (%d,%d).' %(idx,1,best_rank1,best_rank2)
    return best_rank
...
if __name__ == '__main__':
    ...
    indir = '.../BuildData/CUHK01halfData/image/val'
    ...
    for idx in xrange(0,test_num):
        tmp_rank = GetRanks(net,indir,pidx,idx)
