...
def GetRanks(net,indir,pidx,idx,idx2):
    # Load image a
    apath = indir+'/'+'%04d_%02d.jpg' %(idx+1,idx2+1)
    apath_m = indir+'/'+'%04d_%02d_mirror.jpg' %(idx+1,idx2+1)
    if not os.path.exists(apath):
        print 'Image %04d_%02d.jpg is not existed.' %(idx+1,idx2+1)
    if not os.path.exists(apath_m):
        print 'Image %04d_%02d_mirror.jpg is not existed.' %(idx+1,idx2+1)
    
    tmp_ranks=[]
    for n in xrange(0,test_num):
        for n2 in xrange(0,1):
            bpath = indir+'/'+'%04d_%02d.jpg' %(n+1,n2+1)
            bpath_m = indir+'/'+'%04d_%02d_mirror.jpg' %(n+1,n2+1)
            if not os.path.exists(bpath):
                print 'Image %04d_%02d.jpg is not existed.' %(n+1,n2+1)
            if not os.path.exists(bpath_m):
                print 'Image %04d_%02d_mirror.jpg is not existed.' %(n+1,n2+1)
...
def BuildPairFile(indir,ids):
    ...
    for id in xrange(0,ids):
        for i in xrange(0,100):
            tmppath = indir+'/'+'%04d_%02d.jpg' %(id+1,i+1)
            if os.path.exists(tmppath):
                pairidx[id,0] = pairidx[id,0] + 1
            else:
                break
    pospairs = sum(pairidx[:,0])
...
if __name__ == '__main__':
    ...
    indir = '.../BuildData/iLIDSData/image/val'
    ...
    for idx in xrange(0,test_num):
        for idx2 in xrange(1,int(pidx[idx,0])):
